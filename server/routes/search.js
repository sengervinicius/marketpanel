...
router.post('/chat', aiChatKillSwitch, perMinuteLimit, dailyAILimit, aiQuotaGate, async (req, res) => {
  ...
  // W6 Aegea fix: when Perplexity explicitly returns the no-material-news
  // sentinel, we lift that to a named flag so the synthesis prompt can
  // instruct the model NOT to fill the gap with training-data thesis.
...
- When you truly lack live data for a specific asset (it says "no live data available") AND RECENT NEWS is empty or does not mention the entity, DO NOT produce an analytical thesis from your training data. State clearly: "No live terminal data for $TICKER and no fresh news hits in the last 72 hours — I'd check [Valor / Bloomberg Línea / company IR page / regulatory filings] before acting on this name." For Brazilian DEBT-only issuers, name the rating agencies (Fitch / S&P / Moody's) explicitly as sources to check. Producing a training-data thesis on a name that is currently in a credit event, M&A process, or regulatory investigation is a failure mode — silence or a pointer to primary sources is correct
- When you lack live data BUT RECENT NEWS contains material information on the entity (rating action, M&A, litigation, earnings, guidance), LEAD with what the news says. Cite it. Do not bury it under a generic company-overview paragraph
- If RECENT NEWS contains the sentinel "NO MATERIAL NEWS FOUND FOR [entity]", treat that as an explicit signal that the news wire found nothing — do NOT invent coverage and do NOT fill with training-data thesis. State: "No material news for $TICKER in the last 7 days per the news wire."
