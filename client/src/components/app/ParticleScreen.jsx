...
export default function ParticleScreen() {
  ...
  // #218 — per-message copy feedback. Stores the message index (or id) of
  // the most recently copied bubble so we can flash a "Copied" affordance.
  ...
  // ── #218 Per-message share/copy helpers ─────────────────────────────
  //
  // ChatPanel.jsx has these for user→user chat; we duplicate the same
  // logic here for Particle AI messages because the Particle experience
  // is a separate component tree. Keep the helpers idiomatic (clipboard
  // API with textarea fallback for non-secure contexts / older Safari).
  const copyParticleMessage = useCallback(async (msg, idx) => {
    ...
    try {
      if (navigator.clipboard?.writeText) {
        ...
        // Legacy fallback — non-secure contexts block navigator.clipboard.
      ...
      setCopiedMsgIdx(idx);
      setTimeout(() => setCopiedMsgIdx(prev => (prev === idx ? null : prev)), 1600);
    } catch (err) {
      console.warn('Copy failed:', err);
    }
  }, []);

  const emailParticleMessage = useCallback((msg) => {
    const text = (msg?.content || '').trim();
    if (!text) return;
    const subject = encodeURIComponent('Particle AI answer');
    const body = encodeURIComponent(text + '\n\n— Shared from Particle');
    window.open(`mailto:?subject=${subject}&body=${body}`, '_self');
  }, []);

  const whatsappParticleMessage = useCallback((msg) => {
    const text = (msg?.content || '').trim();
    if (!text) return;
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`particle-msg particle-msg--${msg.role}`}>
                ...
                <div className="particle-msg-bubble">
                  ...
                  {/* #218 — per-message share actions (Copy / Email / WhatsApp).
                       Only on finished assistant messages with real content. */}
                  {msg.role === 'assistant' && !msg.streaming && (msg.content || '').trim() && (
                    <div className="particle-msg-actions" role="toolbar" aria-label="Message actions">
                      <button
                        type="button"
                        className="particle-msg-action"
                        onClick={() => copyParticleMessage(msg, i)}
                        title="Copy answer"
                      ...
                      <button
                        type="button"
                        className="particle-msg-action"
                        onClick={() => emailParticleMessage(msg)}
                      ...
                      <button
                        type="button"
                        className="particle-msg-action"
                        onClick={() => whatsappParticleMessage(msg)}
...
function ActionButton({ type, params }) {
  const ACTIONS = {
    ...
    pause_alerts:      { label: 'Pause All',       icon: '\u23F8', color: 'var(--semantic-warn)' },
    enable_alerts:     { label: 'Resume All',      icon: '\u25B6', color: 'var(--color-up)' },
  };
