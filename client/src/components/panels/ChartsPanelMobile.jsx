...
const MobileChart = memo(function MobileChart({ ticker }) {
  ...
  const lineColor = isUp ? '#00c851' : '#f44336';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
      ...
      <div style={{
        ...
        <span style={{ color: '#e0e0e0', fontSize: 15, fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>
          {fmtPrice(price)}
        </span>
        {chg != null && (
      ...
      <div style={{ display: 'flex', gap: 4, padding: '5px 10px', flexShrink: 0 }}>
        {RANGES.map((r, i) => (
          <button key={r.label} onClick={() => setRangeIdx(i)} style={{
            ...
            border: `1px solid ${i === rangeIdx ? 'var(--accent, #ff6600)' : '#1e1e1e'}`,
            color: i === rangeIdx ? 'var(--accent, #ff6600)' : '#555',
            borderRadius: 3,
          }}>{r.label}</button>
        ))}
      </div>
      ...
      <MobileChartContainer>
        {({ width, priceHeight, volumeHeight }) => (
          <>
            {loading ? (
              <div style={{ height: priceHeight + volumeHeight, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#2a2a2a', fontSize: 11 }}>
                Loading...
              </div>
            ) : bars.length === 0 ? (
              <div style={{ height: priceHeight + volumeHeight, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#2a2a2a', fontSize: 11 }}>
              ...
              <>
                <ResponsiveContainer width={width} height={priceHeight}>
                  <AreaChart data={bars} margin={{ top: 6, right: 4, bottom: 0, left: 4 }}>
                    ...
                    <Area
                      type="monotone" dataKey="close" stroke={lineColor} strokeWidth={1.5}
...
function ChartsPanelMobile({ onOpenDetail }) {
  ...
  return (
    <div style={{
      ...
      background: 'var(--bg-app, #0a0a0a)', fontFamily: 'inherit',
    }}>
      {/* Symbol selector bar */}
      <div style={{
        ...
        {chartSymbols.map(sym => (
          <button
            ...
            style={{
              ...
              background: currentSymbol === sym ? 'var(--accent, #ff6600)' : '#111',
              color: currentSymbol === sym ? '#000' : '#888',
              border: `1px solid ${currentSymbol === sym ? 'var(--accent, #ff6600)' : '#2a2a2a'}`,
