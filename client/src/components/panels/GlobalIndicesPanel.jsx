...
const NAMES = {
  ...
  EZU:'EURO STOXX', EWU:'UK FTSE', EWG:'GERMANY DAX', EWQ:'FRANCE CAC', EWP:'SPAIN IBEX', EWI:'ITALY MIB', EWL:'SWITZERLAND', EWD:'SWEDEN',
  EWJ:'JAPAN NIKKEI', EWH:'HONG KONG HSI', EWY:'KOREA KOSPI', EWA:'AUSTRALIA ASX', MCHI:'CHINA CSI', EWT:'TAIWAN', EWS:'SINGAPORE', INDA:'INDIA SENSEX'
};
...
export default function GlobalIndicesPanel() {
  ...
  const fetchData = async () => {
    try {
      ...
      const map = {};
      (json.tickers || []).forEach(t => { map[t.ticker] = t; });
      setData(map);
            ...
            {region.tickers.map((ticker, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span style={{textAlign:'right', color:color(pct), fontWeight:500}}>{fmtPct(pct)}</span>
                </div>
              );
            })}
          </div>
