...
export default function BrazilPanel() {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#0a0a0a' }}>
      {/* Header */}
      <div style={{ padding: '4px 8px', borderBottom: '1px solid #2a2a2a', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
        <span style={{ color: '#e8a020', fontWeight: 700, fontSize: 9, letterSpacing: '0.2em', textTransform: 'uppercase' }}>BRASIL B3</span>
        {error
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {stocks.map(s => {
          ...
          return (
            <div key={s.symbol} style={{ display: 'grid', gridTemplateColumns: '52px 1fr 64px 52px', padding: '3px 8px', borderBottom: '1px solid #111', alignItems: 'center' }}>
              <span style={{ color: '#e8a020', fontWeight: 700, fontSize: 9 }}>{s.symbol}</span>
              <span style={{ color: '#666', fontSize: 8, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.name}</span>
              <span style={{ color: '#ccc', fontSize: 9, textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{fmt(s.price)}</span>
              <span style={{ color: clr, fontSize: 9, textAlign: 'right', fontWeight: 600, fontVariantNumeric: 'tabular-nums' }}>{fmtPct(s.changePct)}</span>
            </div>
          );
        })}
      </div>
